...
export class DataService {
    ...
    private getNodeInternal(nodes: TreeNode[], value: string): TreeNode | null {
        nodes.forEach((node: TreeNode) => {
            ...
            if (node.children && node.children.length > 0) {
                const result: TreeNode | null = this.getNodeInternal(
                    node.children,
                    value
                );
    ...
    public deleteNode(value: string): void {
        const node: TreeNode | undefined = this.nodes.find(
            (node: TreeNode) => node.value === value
        );

        if (node) {
            this.nodes.splice(this.nodes.indexOf(node), 1);
        } else {
            const parent: TreeNode | null = this.getParentForNode(
                this.nodes,
                value
            );

            if (parent) {
                const childNode: TreeNode = parent.children.find(
                    (node: TreeNode) => node.value === value
                ) as TreeNode;
